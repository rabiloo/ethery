...
/**
 * Checks if a rule is a root-level rule (.ethery directory or no file path)
 */
const isRootLevelRule = (rule: RuleWithSource): boolean => {
  return !rule.ruleFile || rule.ruleFile.startsWith(".ethery/");
};
...
export const shouldApplyRule = (
  ...
  // Check if this is a root-level rule (in .ethery directory or no file path)
