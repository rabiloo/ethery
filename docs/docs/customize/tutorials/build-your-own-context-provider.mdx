...
Custom context providers can be implemented in a `config.ts` placed in your Continue global directory (`~/.ethery` for MacOS, `%USERPROFILE%/.ethery` for Windows). You can implement the `CustomContextProvider` interface in your `config.ts`
...
To include outside Node modules in your config.ts, run `npm install <module_name>` from the `~/.ethery` directory, and then import them in config.ts.
